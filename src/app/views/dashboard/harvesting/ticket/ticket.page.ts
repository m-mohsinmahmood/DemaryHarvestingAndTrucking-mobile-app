...
inputClickedTruckDriver() {
  ...
  this.allTruckDrivers = this.harvestingService.getEmployees(this.truckDriverSearchValue,'allEmployees','Truck Driver');
        ...
        // subscribing to show/hide field UL
        this.allTruckDrivers.subscribe((kartoperaotrs) => {
          console.log('kart-operators',kartoperaotrs);
...
kartOperatorSearchSubscription() {
  ...
  this.kart_operator_search$
    ...
    .subscribe((value: string) => {
      ...
      // for asterik to look required
      if(value === ''){ this.isKartOperatorSelected = true;}

     // calling API
       this.allKartOperators = this.harvestingService.getEmployees(this.kartOperatorSearchValue,'allEmployees','Cart Operator');
...
listClickedKartOperator(kartoperator) {
  ...
  // passing name in select's input
  this.kart_operator_name = kartoperator.first_name + ' ' + kartoperator.last_name;
