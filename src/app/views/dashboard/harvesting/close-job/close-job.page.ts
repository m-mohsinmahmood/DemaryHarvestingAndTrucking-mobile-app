...
export class CloseJobPage implements OnInit {
  ...
  initApis() {
    if (this.role === 'crew-chief') {
      this.harvestingService.getBeginningOfDay(localStorage.getItem('employeeId'), 'beginningOfDayHarvesting', 'harvesting');
    }
  ...
  submit() {
    ...
    if (localStorage.getItem('role') === 'crew-chief') {
      let dayClosed = {
        ...
        ending_separator_hours: this.closeJobFormCrew.get("ending_separator_hours").value,
      }

      this.harvestingService.closeBeginningDay(dayClosed)
        .subscribe(
          (res: any) => {
            console.log(res);
            if (res.status === 200) {
